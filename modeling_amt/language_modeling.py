...
def gpt2_associative_forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                outputs, w_mem = torch.utils.checkpoint.checkpoint(
...
def gpt2_lm_associative_forward(
        self,
        input_ids=None,
        past_key_values=None,
        attention_mask=None,
        ...
        transformer_outputs, new_w_mems = self.transformer(
            input_ids,
...
class AssociativeLayerWrapper(torch.nn.Module):
    ...
    def forward(self, hidden_states, w_mem, **kwargs):
        
        mq = self.W_mq(hidden_states) # (bsz, seq_len, d_model)

        hidden_states = mq @ w_mem + hidden_states
        
        out = self.layer(hidden_states=hidden_states, **kwargs)
        
        mem_tokens = out[0][:, -self.num_mem_tokens:]
        new_w_mem = self.update_mem(w_mem, mem_tokens)
        return out, new_w_mem

    def update_mem(self, w_mem, mem_tokens):
        # new_w_mem = w_mem
        ...
        new_w_mem = w_mem + associations
        new_w_mem = new_w_mem / new_w_mem.std(dim=(1, 2))[:, None, None]
        # # log |w_mem| 
        return new_w_mem


class GPT2AssociativeMemoryCell(torch.nn.Module):
    def __init__(self, base_model, num_mem_tokens, d_mem=768, layers_attr: str = 'transformer.h'):
        super().__init__()
        self.model = base_model
        self.model.forward = gpt2_lm_associative_forward.__get__(self.model)
        ...
        for i, attr in enumerate(self.layers_attrs):
            ...
            if i == 0:
                self.layers.forward = gpt2_associative_forward.__get__(self.layers)
    ...
    def forward(self, input_ids, labels=None, labels_mask=None, w_mems=None, **kwargs):

        memory_state = self.set_memory(input_ids.shape)

        seg_kwargs = self.process_input(input_ids, memory_state, **kwargs)

        out, new_w_mems = self.model(w_mems=w_mems, **seg_kwargs)

        out = self.process_output(out, labels, labels_mask, **kwargs)

        return out, new_w_mems
...
class AssociativeRecurrentWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids, labels=None, labels_mask=None, inputs_embeds=None, attention_mask=None, output_attentions=None, output_hidden_states=None, input_segmented=False):
        ...
        cell_outputs = []
        w_mems = torch.zeros(len(self.memory_cell.layers), 1, self.memory_cell.d_mem, self.memory_cell.d_model).to(input_ids.device)

        # freeze model
        for seg_num, segment in enumerate(segmented):
            cell_out, w_mems = self.memory_cell(w_mems=w_mems, **segment, output_hidden_states=True)
